...
class WC_Email_Stock_Alert extends WC_Email {
	...
	function __construct() {
		...
		$this->id 				= 'stock_alert';
		$this->title 			= __( 'Stock Alert', 'woocommerce' );
		$this->description		= __( 'Alert customer when their subscribed product becomes in stock', 'woocommerce' );

		$this->template_html 	= 'emails/stock_alert_email.php';
		$this->template_plain 	= 'emails/plain/stock_alert_email.php';

		$this->subject 			= __( 'Your Subscribed product on {site_title} is available now', 'woocommerce');
...
return new WC_Email_Stock_Alert();
